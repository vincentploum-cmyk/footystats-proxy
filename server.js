...
async function fetchLeagueList() {
  try {
    ...
    for (const league of list) {
      ...
      if (latest) {
        console.log("  " + latest.id + " (" + latest.year + "): " + name);
      }
    }
  } catch (e) {
...
//   S4  Either team FH Over 2.5 % >= 20%                                → 3.46x lift
...
//
// RANK → PROBABILITY (displayed consistently):
//   5 = S1 + S3 + S4 (+ odds booster)   → ~53%
//   4 = S1 + S3 + S4                    → ~44%
//   3 = S1 + S3, or S1 + S4             → ~38%
//   2 = one weaker qualifying setup      → ~22%
  ...
  return {
    ...
    signals: {
      S1: {
        met: S1,
        noData: false,
        label: "Exp FH Goals >= 1.20",
        value: expFH.toFixed(2),
        threshold: ">= 1.20",
        tier: "strong",
      },
      S3: {
        met: S3,
        noData: false,
        label: "Both BTTS FH >= 25%",
        value: h.btts_ht_pct + "%/" + a.btts_ht_pct + "%",
        threshold: "both>=25%",
        tier: "strong",
      },
      S4: {
        met: S4,
        noData: false,
        label: "FH Over 2.5 >= 20%",
        value: h.o25ht_pct + "%/" + a.o25ht_pct + "%",
        threshold: "either>=20%",
        tier: "strong",
      },
      M2: {
        met: M2,
        noData: false,
        label: "FH Clean Sheet <= 30%",
        value: h.cs_ht_pct + "%/" + a.cs_ht_pct + "%",
        threshold: "either<=30%",
        tier: "medium",
      },
      M4: {
        met: M4,
        noData: false,
        label: "Early Goals >= 0.25/gm",
        value:
          h.cn010_avg.toFixed(2) + "/" + a.cn010_avg.toFixed(2),
...
function extractSnapshotStats(teamObj, role) {
  ...
  return {
    ...
    scored_fh: safe(
      s["scoredAVGHT" + sfx] || s.scoredAVGHT_overall || 0
    ),
    conced_fh: safe(
      s["concededAVGHT" + sfx] || s.concededAVGHT_overall || 0
    ),
    btts_ht_pct: safe(
      s["seasonBTTSPercentageHT" + sfx] ||
        s.seasonBTTSPercentageHT_overall ||
        0
    ),
    cs_ht_pct: safe(
      s["seasonCSPercentageHT" + sfx] ||
        s.seasonCSPercentageHT_overall ||
        0
    ),
    o25ht_pct: safe(
      s["seasonOver25PercentageHT" + sfx] ||
        s.seasonOver25PercentageHT_overall ||
        0
    ),
    o15ht_pct: safe(
      s["seasonOver15PercentageHT" + sfx] ||
        s.seasonOver15PercentageHT_overall ||
        0
    ),
    fts_ht_pct: safe(
      s["seasonFTSPercentageHT" + sfx] ||
        s.seasonFTSPercentageHT_overall ||
        0
    ),
    cn010_avg: safe(
      safeDiv(s["goals_conceded_min_0_to_10" + sfx] || 0, mpR)
    ),
  ...
  try {
    ...
    for (const sid of Object.keys(leagueFixtures)) {
      ...
      try {
        const p1 = await ftch(
          BASE +
            "/league-matches?season_id=" +
            sid +
            "&max_per_page=100&page=1&key=" +
            KEY
        );
      ...
      try {
        ...
        if (!Object.keys(teamMap).length && PREV_SEASON[sid]) {
          const tr2 = await ftch(
            BASE +
              "/league-teams?season_id=" +
              PREV_SEASON[sid] +
              "&include=stats&key=" +
              KEY
          );
      ...
      for (const fixture of fixtures) {
        ...
        if (!missing) {
          ...
          const odds_fh_o15 =
            parseFloat(fixture.odds_1st_half_over15 || 0) || null;
          const odds_ft_o25 =
            parseFloat(fixture.odds_ft_over25 || 0) || null;
        ...
        const hLast5 = homeId
          ? buildLast5(homeId, "home", completedMatches)
          : [];
        const aLast5 = awayId
          ? buildLast5(awayId, "away", completedMatches)
          : [];
        const h2h =
          homeId && awayId
            ? buildH2H(homeId, awayId, completedMatches)
            : [];
        ...
        preds.push({
          ...
          result: isComplete
            ? {
                ...
                hit:
                  parseInt(fixture.ht_goals_team_a || 0, 10) +
                    parseInt(fixture.ht_goals_team_b || 0, 10) >
                  2,
              }
            : null,
        });
