...
// ── Main route ────────────────────────────────────────────────────────────────
// Convert a unix timestamp + tzOffset (minutes) to a "YYYY-MM-DD" string in the user's local day
const unixToLocalDate=(unix,tzOffset)=>{
  if(!unix) return null;
...
function buildHTML(preds,dates){
  ...
  // FH avg stats — immediately after team names
  H+="html+=statsBar;";
  H+="html+='<div style=\"display:flex;align-items:center;gap:6px;margin-top:8px\">'+dots+'<span style=\"font-size:12px;color:#6b7280\">'+m.nMet+'/4 signals</span></div>';";
  H+="html+='</div>';";
  // Score / prob badge
