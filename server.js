...
function parseLastMatches(res, teamId) {
  const matches = res && res.data ? res.data : [];
  return matches.slice(0, 5).map((m) => {
    const isHome =
      String(m.homeID || m.home_id || "") === String(teamId);
...
app.get("/", async (req, res) => {
  try {
    ...
    // Build team maps per active league
    ...
    // Fetch /lastx once per unique team, not once per fixture
    const lastxMap = {};
    await Promise.all(
      [...uniqueTeamIds].map(async (teamId) => {
        try {
          const r = await ftch(BASE + "/lastx?team_id=" + teamId + "&key=" + KEY);
          lastxMap[teamId] = parseLastMatches(r, teamId);
        } catch (e) {
          console.log("lastx error team_id=" + teamId + ": " + e.message);
          lastxMap[teamId] = [];
        }
      })
    );
    ...
    for (const sid of Object.keys(leagueFixtures)) {
      ...
      for (const fixture of leagueFixtures[sid] || []) {
        ...
        const hTeamId = htRaw && htRaw.id != null ? String(htRaw.id) : null;
        const aTeamId = atRaw && atRaw.id != null ? String(atRaw.id) : null;

        const hLast5 = hTeamId ? (lastxMap[hTeamId] || []) : [];
        const aLast5 = aTeamId ? (lastxMap[aTeamId] || []) : [];
