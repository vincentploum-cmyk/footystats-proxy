...
app.get("/debug-league",async(req,res)=>{
  ...
  try{
    ...
    res.json({
      ...
      sampleTeamTopKeys:Object.keys(sampleTeam).slice(0,20),
      sampleTeamStatsKeys:sampleTeam.stats?Object.keys(sampleTeam.stats).slice(0,40):[],
      sampleTeamHtStats:sampleTeam.stats?
        Object.fromEntries(Object.entries(sampleTeam.stats).filter(([k])=>k.includes('HT')||k.includes('ht'))):{}
    });
  }catch(e){res.status(500).json({error:e.message})}
});
...
function calcAV(ht,at){
  const hVal=safe(ht.AVGHT_home||ht.AVGHT_overall);
  const aVal=safe(at.AVGHT_away||at.AVGHT_overall);
  const combined=hVal+aVal;
  return{hVal,aVal,combined,met:combined>=1.6};
}
...
app.get("/",async(req,res)=>{
  try{
    ...
    for(const d of dates){
      try{
        const r=await ftch(BASE+"/todays-matches?date="+d+"&key="+KEY);
        for(const m of (r.data||[])) allFixtures.push(Object.assign({},m,{_date:d}));
      }catch(e){console.log("fixtures error date="+d,e.message);}
    }
    ...
    for(const sid of Object.keys(leagueFixtures)){
      ...
      try{
        ...
        for(const t of (r.data||[])){
          const flat=t.stats&&typeof t.stats==="object"?Object.assign({},t,t.stats):t;
          if(t.id!=null){teamMap[t.id]=flat;teamMap[String(t.id)]=flat;}
          teamMap["__name__"+(t.name||"").toLowerCase().trim()]=flat;
          if(t.clean_name)teamMap["__name__"+t.clean_name.toLowerCase().trim()]=flat;
        }
      }catch(e){console.log("league-teams error sid="+sid,e.message);}

      if(!Object.keys(teamMap).length){continue;}
      ...
      for(const fixture of (leagueFixtures[sid]||[])){
        ...
        const ht=teamMap[homeId]||teamMap[parseInt(homeId)]||teamMap["__name__"+(fixture.home_name||"").toLowerCase().trim()]||{};
        const at=teamMap[awayId]||teamMap[parseInt(awayId)]||teamMap["__name__"+(fixture.away_name||"").toLowerCase().trim()]||{};

        const missingStats=(!teamMap[homeId]&&!teamMap["__name__"+(fixture.home_name||"").toLowerCase().trim()])
                         ||(!teamMap[awayId]&&!teamMap["__name__"+(fixture.away_name||"").toLowerCase().trim()]);
        ...
        const signals=[
          {
            key:"CI",label:"HT Intensity Index (CI)",
            ...
            combinedVal:ci.val.toFixed(3),threshold:">= 3.2",met:ci.met,lift:"2.76x lift"
          },
          {
            key:"T1",label:"Both Teams FH Over 2.5%",
            ...
            threshold:"both >= 20%",met:t1.met,lift:"2.65x lift"
          },
          {
            key:"O1",label:"Both Teams FH Over 1.5 Rate",
            desc:"seasonOver15PercentageHT_overall >= 35% for both — 2.08x lift",
            ...
            threshold:"both >= 35%",met:o1.met,lift:"2.08x lift"
          },
          {
            key:"AV",label:"Combined FH Goals Avg",
            desc:"AVGHT_home(H) + AVGHT_away(A) >= 1.6",
            hVal:av.hVal.toFixed(2)+" avg",aVal:av.aVal.toFixed(2)+" avg",
            combinedVal:av.combined.toFixed(3),threshold:"sum >= 1.6",met:av.met,lift:"volume signal"
          }
        ];
        ...
        preds.push({
          ...
          prob,nMet,signals,missingStats,
          status:fixture.status||"incomplete",
          fhH:parseInt(fixture.ht_goals_team_a||0),fhA:parseInt(fixture.ht_goals_team_b||0),
          ftH:parseInt(fixture.homeGoalCount||0),ftA:parseInt(fixture.awayGoalCount||0)
        });
      }
    }
...
function buildHTML(preds,dates){
  ...
  H+="0 signals = 10% &nbsp;|&nbsp; 1 = 20% &nbsp;|&nbsp; 2 = 25% &nbsp;|&nbsp; 3 = 35% &nbsp;|&nbsp; 4 = 45%. All stats-based, no odds.";
  H+="</div>";
  ...
  H+="var DAY_LABELS=[\"Today\",\"Tomorrow\",\"Day 3\",\"Day 4\"];";
  ...
  H+="  if(!leagueList.length){document.getElementById('mainView').innerHTML='<div style=\"background:#fff;border:1px solid #e5e7eb;border-radius:8px;padding:40px;text-align:center;color:#6b7280\">No matches found.</div>';return;}";
  H+="  var html='<div style=\"font-size:13px;color:#6b7280;margin-bottom:12px\">'+dayPreds.length+' matches across '+leagueList.length+' leagues</div>';";
  ...
  H+="    html+='<div class=\"league-card\" onclick=\"selectLeague('+JSON.stringify(league)+')\">';";
  H+="    html+='<div style=\"flex:1;min-width:0;margin-right:12px\"><div style=\"font-size:18px;font-weight:700;color:#111827;margin-bottom:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis\">'+league+'</div>';";
  H+="    html+='<div style=\"font-size:13px;color:#6b7280\">'+matches.length+' match'+(matches.length>1?'es':'')+hotStr+'</div></div>';";
  H+="    html+='<div style=\"text-align:right;flex-shrink:0\"><div style=\"font-size:26px;font-weight:800;color:'+probCol+'\">'+maxProb+'%</div>';";
  H+="    html+='<div style=\"font-size:11px;color:#9ca3af;margin-top:1px\">'+maxN+'/4 signals</div></div></div>';";
  H+="  }";
  ...
  H+="  var dots='';";
  H+="  for(var i=0;i<m.signals.length;i++){var sc=m.signals[i].met?'#16a34a':'#e5e7eb';dots+='<span title=\"'+m.signals[i].label+'\" style=\"display:inline-block;width:12px;height:12px;border-radius:50%;background:'+sc+';margin-right:3px\"></span>';}";
  H+="  var sigRows='';";
  ...
  H+="  html+='<div style=\"min-width:0\"><div style=\"font-size:12px;color:#9ca3af;margin-bottom:3px\">'+dt+warnStr+'</div>';";
  ...
  H+="  html+='<div style=\"display:flex;align-items:center;gap:8px\">'+dots+'<span style=\"font-size:12px;color:#6b7280\">'+m.nMet+'/4 signals met</span></div></div>';";
  ...
  H+="  html+='<div style=\"padding-top:10px\"><table class=\"sig-table\" style=\"margin-bottom:14px\">';";
  H+="  html+='<thead><tr><th style=\"width:30%\">Signal</th><th style=\"width:16%;text-align:center\">'+shortName(m.home)+'</th>';";
  H+="  html+='<th style=\"width:16%;text-align:center\">'+shortName(m.away)+'</th>';";
  H+="  html+='<th style=\"width:18%;text-align:center\">Combined</th><th style=\"width:20%;text-align:center\">Result</th>';";
