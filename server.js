  ...
  try{
    ...
    for(const sid of Object.keys(leagueFixtures)){
      ...
      for(const fixture of (leagueFixtures[sid]||[])){
        ...
        // Fetch last 5 matches per team (/lastx endpoint)
        let hLast5=[], aLast5=[];
        try{
          const homeTeamId=htRaw?htRaw.id:null;
          const awayTeamId=atRaw?atRaw.id:null;
          const parseLast=(res,teamId)=>{
            const matches=(res&&res.data)?res.data:[];
            return matches.slice(0,5).map(m=>{
              const isHome=String(m.homeID||m.home_id||"")===String(teamId);
              return{
          ...
          const [hRes,aRes]=await Promise.all([
            homeTeamId?ftch(BASE+"/lastx?team_id="+homeTeamId+"&key="+KEY):Promise.resolve(null),
            awayTeamId?ftch(BASE+"/lastx?team_id="+awayTeamId+"&key="+KEY):Promise.resolve(null)
          ]);
          hLast5=parseLast(hRes,homeTeamId);
          aLast5=parseLast(aRes,awayTeamId);
        }catch(e){console.log("lastx error: "+e.message);}
