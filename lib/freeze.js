...
// A match's result may only be RECORDED (shown as final / persisted) once it is
// genuinely final. FootyStats marks confirmed finals `complete`. Some leagues never
// flip a played match off `incomplete`, so an `incomplete` match counts as final ONLY
// when it is BOTH past kickoff (isPlayed) AND carries a posted score (ftGoals > 0).
// A 0-0 `incomplete` match is almost always "score not posted yet", not a real 0-0 —
// recording it would lock in a placeholder 0-0 (a false MISS) that a later real score
// can't overwrite (backfill only touches hit_25 IS NULL rows). So it stays pending.
function isResultFinal(status, isPlayed, ftGoals) {
  if (status === "complete") return true;
  if (status === "incomplete") return !!isPlayed && Number(ftGoals) > 0;
  return false;
}

module.exports = { computeOverCandidates, selectPregamePrediction, isResultFinal };
